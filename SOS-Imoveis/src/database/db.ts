...
export const clearLocalData = async (): Promise<void> => {
  try {
    ...
    db.execSync('DELETE FROM brokers;');
    db.execSync('DELETE FROM clients;');
    db.execSync('DELETE FROM properties;');
    db.execSync('DELETE FROM contracts;');
    db.execSync('DELETE FROM property_reviews;');
    db.execSync('DELETE FROM property_favorites;');
  } catch {
    // ignore cleanup errors on unsupported tables
  }
};
